...
Или же коэффициент загруженности - это отношение размера таблицы к её текущей вместимости. Чем он больше, тем больше вероятность коллизий, что, разумеется, плохо, тк ведёт к увеличению времени действий. А хеширование цепочками позволяет достичь больших значений этого коэффициента.
 ...
 При достижении выбранного коэффициента, таблица рехешируется вручную (т.е. пользователем в main), то есть её вместимость увеличивается и заново считаются хеши для ключей в таблице.
...
2. perf record -g ./<название программы> && perf report --no-children (вторая консоль не нужна, покажет те же данные, только статично, более подробно и измеренные на протяжении всей программы)
...
Запустив: perf record -g ./HashTable && perf report --no-children, получим:
...
Тогда готовая функция выглядит так:
...
Ещё раз профилируем программу с помощью 

        perf record -g ./HashTable && perf report --no-children
Получаем список нескольких самых ресурсо-потребляющих функций:
![alt text](Pictures/Report2.png)
...
1. [Лекция ВМК МГУ](http://algcourse.cs.msu.su/wp-content/uploads/2010/12/Lect13.pdf)
2. [Crc_32](https://github.com/gcc-mirror/gcc/blob/master/libiberty/crc32.c)
...
### Измерение
!Все измерения проводятся с флагом ускорения -O0!
...
2) Линковка. Подробнее в главном Makefile.

Теперь измерим по 7 раз с помощью perf stat, получим средний результат:
|   Номер теста |   1    |   2    |   3    |   4    |   5    |   6    |   7    |
|---------------|--------|--------|--------|--------|--------|--------|--------|
|   время       |440.01  |454.80  |458.25  |434.52  |453.25  |458.31  |425.38  |

Среднее значение: $446,36$ ms

Ускорение в $k = \frac{476,80}{446,36} = 1,0682$, то есть на $6,82$%
...
Оптимизируем одно из самых узких мест: strcmp. Перепишем её с помощью встроенного ассемблера:
        ...
        int AsmStrcmp(const char *a, const char *b)
        {
            ...
            return (1-result); //дла соотвествия strcmp
        }

Теперь измерим 7 раз с помощью perf stat, получим средний результат:
|   Номер теста |   1    |   2    |   3    |   4    |   5    |   6    |   7    |
|---------------|--------|--------|--------|--------|--------|--------|--------|
|   время       |465.84  |461.51  |453.17  |448.77  |473.22  |447.85  |445.30  |

Среднее значение: $456,52$ ms

Ускорение в $k = \frac{446,36}{456,52} = 0,9777$, то есть программа замедлилиась на $2,23$%. Такое бывает, да, это грустно, но помогает понять: не нужно оптимизировать всё подряд!

## Вывод
Программа ускорилась в $1,1296$ раз, если убрать последнюю оптимизацию. Посчитаем коэффициент $\mu = \frac{ускорение}{\text{строки на ассемблере}} = \frac{0,1296}{1 + 48} = 2,6449 * 10^{-3}$, что эквивалентно $0,2645$ процентам.